...
public class MCPPromptsService
{
    ...
    private static object GetPromptVariables(string templateText)
    {
        ...
        var matches = ExtractTemplateVariables(templateText);
    ...
    private static System.Text.RegularExpressions.MatchCollection ExtractTemplateVariables(string templateText)
    {
        const string variablePattern = @"\{\{(\w+)\}\}";
        return System.Text.RegularExpressions.Regex.Matches(templateText, variablePattern);
    }
    
    private static string FormatVariableLabel(string variableName)
    {
        // Convert camelCase/snake_case to human readable format
        var result = ConvertCamelCaseToSpaces(variableName);
    ...
    {
        const string camelCasePattern = @"([a-z])([A-Z])";
        return System.Text.RegularExpressions.Regex.Replace(input, camelCasePattern, "$1 $2");
    }
